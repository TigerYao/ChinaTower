...
function _tmp() {
  const data = _interopRequireDefault(require("tmp"));
...
function _isobject() {
  const data = _interopRequireDefault(require("isobject"));

  _isobject = function _isobject() {
    return data;
  };
...
function _pathIsInside() {
  ...
  _pathIsInside = function _pathIsInside() {
    return data;
  };
...
function _importFresh() {
  const data = _interopRequireDefault(require("import-fresh"));

  _importFresh = function _importFresh() {
    return data;
  };
...
function _validIdentifier() {
  const data = _interopRequireDefault(require("valid-identifier"));

  _validIdentifier = function _validIdentifier() {
    return data;
  };
...
function _cordovaFetch() {
  const data = _interopRequireDefault(require("cordova-fetch"));

  _cordovaFetch = function _cordovaFetch() {
    return data;
  };
...
const CordovaLogger = _cordovaCommon().CordovaLogger.get();

const DEFAULT_VERSION = '1.0.0';

let events = _cordovaCommon().events;
/**
...
function setupEvents(externalEventEmitter) {
  if (externalEventEmitter) {
    ...
    events.forwardEventsTo(externalEventEmitter); // There is no logger if external emitter is not present,
...
var _default = cordovaCreateLegacyAdapter;
/**
...
function cordovaCreate(dest, opts = {}) {
  ...
  return Promise.resolve().then(function () {
    if (!dir) {
      throw new (_cordovaCommon().CordovaError)('Directory not specified. See `cordova help`.');
    }

    if (!(0, _isobject().default)(opts)) {
      throw new (_cordovaCommon().CordovaError)('Given options must be an object');
    } // Shallow copy opts


    opts = Object.assign({}, opts);
    events = setupEvents(opts.extEvents);
    events.emit('verbose', 'Using detached cordova-create'); // Make absolute.

    dir = _path().default.resolve(dir); // if (fs.existsSync(dir) && fs.readdirSync(dir).length > 0) {
    //     throw new CordovaError('Path already exists and is not empty: ' + dir);
    // }

    if (opts.id && !(0, _validIdentifier().default)(opts.id)) {
      throw new (_cordovaCommon().CordovaError)('App id contains a reserved word, or is not a valid identifier.');
    ...
    if ((0, _pathIsInside().default)(dir, opts.url)) {
      throw new (_cordovaCommon().CordovaError)(`Cannot create project "${dir}" inside the template used to create it "${opts.url}".`);
    }
  }).then(function () {
    // Finally, Ready to start!
    events.emit('log', 'Creating a new cordova project.'); // If symlink, don't fetch
    ...
    if (opts.template && isRemoteUri(opts.url)) {
      ...
      return (0, _cordovaFetch().default)(target, getSelfDestructingTempDir(), {});
    } else {
      // If assets are not online, resolve as a relative path on local computer
      return _path().default.resolve(opts.url);
    }
  }).then(function (input_directory) {
    let import_from_path = input_directory; // handle when input wants to specify sub-directory (specified in index.js as "dirname" export);

    let isSubDir = false;

    try {
      const templatePkg = (0, _importFresh().default)(input_directory);
    ...
    if (!_fsExtra().default.existsSync(import_from_path)) {
      throw new (_cordovaCommon().CordovaError)('Could not find directory: ' + import_from_path);
    }

    const dirAlreadyExisted = _fsExtra().default.existsSync(dir);
    ...
    try {
      ...
      copyIfNotExists(stockAssetPath('www'), _path().default.join(dir, 'www'));
      copyIfNotExists(stockAssetPath('hooks'), _path().default.join(dir, 'hooks'));
      const configXmlExists = projectConfig(dir); // moves config to root if in www

      if (!configXmlExists) {
        _fsExtra().default.copySync(stockAssetPath('config.xml'), _path().default.join(dir, 'config.xml'));
      }
    } catch (e) {
      if (!dirAlreadyExisted) {
      ...
      if (process.platform.slice(0, 3) === 'win' && e.code === 'EPERM') {
        throw new (_cordovaCommon().CordovaError)('Symlinks on Windows require Administrator privileges');
      }
    ...
    _fsExtra().default.ensureDirSync(_path().default.join(dir, 'platforms'));

    _fsExtra().default.ensureDirSync(_path().default.join(dir, 'plugins'));

    const configPath = _path().default.join(dir, 'config.xml'); // only update config.xml if not a symlink


    if (!_fsExtra().default.lstatSync(configPath).isSymbolicLink()) {
      // Write out id, name and default version to config.xml
      const conf = new (_cordovaCommon().ConfigParser)(configPath);
...
function copyIfNotExists(src, dst) {
  if (!_fsExtra().default.existsSync(dst) && src) {
    _fsExtra().default.copySync(src, dst);
  }
}
/**
...
function copyTemplateFiles(templateDir, projectDir, isSubDir) {
  let copyPath = ''; // if template is a www dir

  if (_path().default.basename(templateDir) === 'www') {
    copyPath = _path().default.resolve(templateDir);

    _fsExtra().default.copySync(copyPath, _path().default.resolve(projectDir, 'www'));
  } else {
    let templateFiles = _fsExtra().default.readdirSync(templateDir); // Remove directories, and files that are unwanted
    ...
    templateFiles.forEach(f => {
      copyPath = _path().default.resolve(templateDir, f);

      _fsExtra().default.copySync(copyPath, _path().default.resolve(projectDir, f));
    });
  }
}
/**
...
function projectConfig(projectDir) {
  const rootPath = _path().default.join(projectDir, 'config.xml');

  const wwwPath = _path().default.join(projectDir, 'www', 'config.xml');

  if (_fsExtra().default.existsSync(rootPath)) {
    return rootPath;
  } else if (_fsExtra().default.existsSync(wwwPath)) {
    _fsExtra().default.renameSync(wwwPath, rootPath);
...
function linkFromTemplate(templateDir, projectDir) {
  ...
  function rmlinkSync(src, dst, type) {
    if (src && dst) {
      _fsExtra().default.removeSync(dst);

      if (_fsExtra().default.existsSync(src)) {
        _fsExtra().default.symlinkSync(src, dst, type);
      }
    }
  ...
  if (_path().default.basename(templateDir) === 'www') {
    linkSrc = _path().default.resolve(templateDir);
    linkDst = _path().default.join(projectDir, 'www');
    rmlinkSync(linkSrc, linkDst, 'dir');
    copySrc = _path().default.join(templateDir, 'config.xml');
  } else {
    linkFolders = ['www', 'merges', 'hooks']; // Link each folder

    for (let i = 0; i < linkFolders.length; i++) {
      linkSrc = _path().default.join(templateDir, linkFolders[i]);
      linkDst = _path().default.join(projectDir, linkFolders[i]);
      rmlinkSync(linkSrc, linkDst, 'dir');
    }

    linkSrc = _path().default.join(templateDir, 'config.xml');
    linkDst = _path().default.join(projectDir, 'config.xml');
    rmlinkSync(linkSrc, linkDst, 'file');
    copySrc = _path().default.join(templateDir, 'www', 'config.xml');
  } // if template/www/config.xml then copy to project/config.xml


  copyDst = _path().default.join(projectDir, 'config.xml');

  if (!_fsExtra().default.existsSync(copyDst) && _fsExtra().default.existsSync(copySrc)) {
    _fsExtra().default.copySync(copySrc, copyDst);
  }
}

function stockAssetPath(p) {
  return _path().default.join(require('cordova-app-hello-world').dirname, p);
...
function isRemoteUri(uri) {
  return (0, _isUrl().default)(uri) || uri.includes('@') || !_fsExtra().default.existsSync(uri);
}