...
package com.chinatower.fghd.customer.widget;
...
public class BatteryView extends View {
    ...
    private void drawHorizontalBattery(Canvas canvas) {
        ...
        int gap = 10;
        RectF r2 = new RectF(strokeWidth + gap, strokeWidth + gap, offset - gap, height - strokeWidth - gap);
        //根据电池电量决定电池内矩形电量颜色
        if (mPower < 30) {
    ...
    private void drawVerticalBattery(Canvas canvas) {
        ...
        paint.setColor(mColor);
        paint.setStyle(Paint.Style.STROKE);
        float strokeWidth = height / 15.0f;
        float strokeWidth2 = strokeWidth / 2;
        paint.setStrokeWidth(strokeWidth);

        int headHeight = (int) (strokeWidth + 2.0f);
        ...
        int gap = 3;
        RectF rect2 = new RectF(strokeWidth + gap, headHeight + strokeWidth + topOffset + gap, width - strokeWidth - gap, height - strokeWidth - 2);
        paint.setStyle(Paint.Style.FILL);
        //根据电池电量决定电池内矩形电量颜色
        int powerColor = 0;
        if (isOnLine) {
            if (mPower >= 30) {
                powerColor = this.getContext().getResources().getColor(R.color.main_theme);
            } else if (mPower >= 20 && mPower < 30) {
                powerColor = this.getContext().getResources().getColor(R.color.power_20_30);
            } else {
                powerColor = this.getContext().getResources().getColor(R.color.power_down_20);
            }
        } else {
            powerColor = this.getContext().getResources().getColor(R.color.power_off_line);
